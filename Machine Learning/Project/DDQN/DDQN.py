...
class DDQNAgent:
    ...
    def compute_td_loss(self, states, actions, rewards, next_states, is_done, gamma=0.99):
        ...
        predicted_qvalues = self.DQN(states) # q-values
        predicted_qvalues_for_actions = predicted_qvalues[range(states.shape[0]), actions] # select q-values
        predicted_next_qvalues_current = self.DQN(next_states) # compute q-values
        predicted_next_qvalues_target = self.DQN_target(next_states)
        next_state_values = predicted_next_qvalues_target.gather(1, torch.max(predicted_next_qvalues_current, 1)[1].unsqueeze(1)).squeeze(1) # V*(next_states)
        target_qvalues_for_actions = rewards + gamma * next_state_values  # "target q-values" for loss
